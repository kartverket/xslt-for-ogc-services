...
<xsl:template match="Layer">
  ...
  <xsl:variable name="client">
    http://labs.kartverket.no/norgeskart/wms.html?id=<xsl:value-of select="Name"/>&amp;url=<xsl:value-of select="$url"/>&amp;name=<xsl:value-of select="Title"/>&amp;layers=<xsl:value-of select="Name"/>#<xsl:call-template name="selectScale"/>/189721/6833548
  </xsl:variable>
